...
public class DefaultFuture extends CompletableFuture<Object> {
    ...
    public static DefaultFuture newFuture(Channel channel, Request request, int timeout, ExecutorService executor) {
        ...
        // ThreadlessExecutor 需要持有请求的 DefaultFuture，主要用于
    ...
    public static void received(Channel channel, Response response, boolean timeout) {
        try {
            ...
            if (future != null) {
                ...
                System.out.println("--------------------");
                System.out.println("没有找到 DefaultFuture");
                System.out.println("-------------");
                logger.error("The timeout response finally returned at "
    ...
    private void doReceived(Response res) {
        ...
        // 已经有了请求结果，但是可能还有线程在 ThreadlessExecutor.queue 上等待。为了避免一直等待，需要通知调用线程返回。
    ...
    private static class TimeoutCheckTask implements TimerTask {
        ...
        public void run(Timeout timeout) {
            // 根据请求id取出对应的 DefaultFuture
            DefaultFuture future = DefaultFuture.getFuture(requestID);

            // 判断 timeout 到了，结果是否返回，没有返回则请求超时了
            if (future == null || future.isDone()) {
                return;
            }
