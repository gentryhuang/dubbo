...
public abstract class AbstractRouterRule {
    ...
    /**
     * 表示是否在每次调用时执行该路由规则。
     * 如果设置为 false，则会在 Provider 列表变更时预先执行并缓存结果，调用时直接从缓存中获取路由结果。
     */
    ...
    /**
     * 范围：application/service
     */
    private String scope;
    /**
     * 明确规则体作用在哪个服务或应用
     * 1 scope 为 service: 由 [{group}:]{service}[:{version}] 构成
     * 2 scope 为 application: application
     */
    private String key;
