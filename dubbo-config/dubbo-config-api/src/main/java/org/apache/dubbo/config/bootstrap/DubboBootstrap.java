...
public class DubboBootstrap extends GenericEventListener {
    ...
    public DubboBootstrap start() {
        ...
        if (started.compareAndSet(false, true)) {
            // 用于判断当前节点是否已经启动完毕，在 Dubbo QoS中会使用到该字段
            ready.set(false);
            // 初始化一些基础组件，例如，配置中心相关组件、事件监听、元数据相关组件 等
            initialize();
            if (logger.isInfoEnabled()) {
            ...
            // 处理 Consumer的ReferenceConfig，即服务引用流程
            referServices();
     ...
     * 服务发布入口
     * 说明：
     * 其中每一个服务接口都会转换为对应的 ServiceConfig，然后通过代理的方式转换为 Invoker，最终转换成 Exporter 进行发布
     */
    ...
    private void referServices() {
        ...
        // 从 ConfigManager 中获取所有 ReferenceConfig 列表，并根据 ReferenceConfig 获取对应的代理对象
        configManager.getReferences().forEach(rc -> {
            // TODO, compatible with  ReferenceConfig.refer()
            ReferenceConfig referenceConfig = (ReferenceConfig) rc;
            referenceConfig.setBootstrap(this);

            // 检测 ReferenceConfig 是否已经初始化
            if (rc.shouldInit()) {
                // 异步
                if (referAsync) {
